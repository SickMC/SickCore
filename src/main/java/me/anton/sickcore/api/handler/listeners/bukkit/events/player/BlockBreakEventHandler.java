...
public class BlockBreakEventHandler extends BukkitEventProvider<BlockBreakEvent> {
    @EventHandler
    public void handleEvent(BlockBreakEvent event) {
        ...
        provider.iterator(run -> run.onBlockBreak(event.getBlock(), event, bukkitPlayer));
    }
}
